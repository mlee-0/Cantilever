...
def write_samples(samples):
    ...
    for load_sample, angle_sample in zip(*samples[0:2]):
        ...
        load_components.append((
            np.cos(angle_sample) * load_sample / (NUMBER_DIVISIONS[1]),
            np.sin(angle_sample) * load_sample / (NUMBER_DIVISIONS[1]),
            ))
...
def convert_fea_to_array(samples):
    ...
    for i, filename in enumerate(filenames):
        ...
        with Image.fromarray(array_scaled, 'L') as image:
            image = image.resize((
                round((length_samples[i] / length.high) * OUTPUT_SIZE[0]),
                round((height_samples[i] / height.high) * OUTPUT_SIZE[1]),
                ))
            array_scaled = np.asarray(image, float)
