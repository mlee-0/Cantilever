...
from main import NUMBER_SAMPLES, load, angle, length, height, MESH_DIVISIONS, OUTPUT_SIZE, FOLDER_ROOT, FOLDER_TRAIN_INPUTS, FOLDER_TRAIN_OUTPUTS, NUMBER_DIGITS, generate_samples, generate_input_images, calculate_load_components, write_ansys_script
...
def convert_fea_to_spreadsheet(samples, filename):
    length_samples, height_samples = samples[2:4]
    fea_filenames = glob.glob(os.path.join(FOLDER_TRAIN_OUTPUTS, '*.txt'))
    fea_filenames = sorted(fea_filenames)
    assert len(fea_filenames) == NUMBER_SAMPLES, f'Found {len(fea_filenames)} .txt files in {FOLDER_TRAIN_OUTPUTS}, but should be {len(length_samples)}.'
    ...
    filepath = os.path.join(FOLDER_TRAIN_OUTPUTS, filename)
