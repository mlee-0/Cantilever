...
class AutoencoderCnn(nn.Module):
    def __init__(self):
        ...
        self.convolution_1 = convolution(
            in_channels=INPUT_CHANNELS, out_channels=64, kernel_size=3, stride=3, padding=0
            )
        self.convolution_2 = convolution(
            in_channels=64, out_channels=128, kernel_size=3, stride=3, padding=1
            )
        self.deconvolution_1 = deconvolution(
            in_channels=128, out_channels=64, kernel_size=3, stride=3, padding=1, output_padding=(2,0)
            )
        self.deconvolution_2 = deconvolution(
            in_channels=64, out_channels=OUTPUT_CHANNELS, kernel_size=3, stride=3, padding=0, output_padding=(2,1)
            )
        self.pooling = nn.MaxPool2d(kernel_size=2, stride=2, return_indices=True)
        self.unpooling = nn.MaxUnpool2d(kernel_size=2, stride=2)
        ...
        self.autoencoder = nn.Sequential(
            # nn.ReLU(),
            nn.Linear(in_features=384, out_features=24),
            ...
            nn.Linear(in_features=24, out_features=384),
            # nn.ReLU(),
            )
    ...
    def forward(self, x):
        x = x.float()
        # # Pad the bottom, left, and right to make the dimensions divisible by the stride.
        # x = nn.functional.pad(x, (0, 1, 1, 1))
        # Convolution.
        x = self.convolution_1(x)
        x = self.dropout(x)
        size_1 = x.size()
